...
app.put('/routes', (req, res) => {
    ...
    // Validate routes
    const validRoutes = routes.filter((r) => r.claudeTier && r.providerName && r.targetModel);
...
async function loadConfigOnStartup() {
    ...
    try {
        ...
        const timestampedResults = new Map(Array.from(validationResults.entries()).map(([name, result]) => [
            name,
 ...
 */
export async function startServer(port = DEFAULT_PORT, host = DEFAULT_HOST) {

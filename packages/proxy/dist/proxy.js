 ...
 */
function emitAnthropicError(res, error) {
...
export async function handleProxyRequest(req, res) {
    ...
    const adapter = getOrCreateAdapter(providerType, resolution.provider.baseUrl);
    ...
    let retryAttempt = 0;
    try {
        const upstreamResponse = await fetchWithRetry(resolution.provider.name, async (attemptNumber) => {
            retryAttempt = attemptNumber;
            const controller = new AbortController();
            const timeout = setTimeout(() => controller.abort(), adapter.timeouts.streaming);
            const response = await fetch(`${resolution.provider.baseUrl}${adapter.apiPath}`, {
                method: 'POST',
                headers: {
        ...
        if (retryAttempt > 0) {
            req._retryAttempt = retryAttempt;
        }

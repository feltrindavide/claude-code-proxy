...
router.post('/providers', async (req, res) => {
    try {
        ...
        let validation;
        try {
            validation = await providerValidatorService.validateProvider(name, baseUrl);
            if (!validation.valid) {
                console.warn(`[Admin] Provider validation warning for ${name}: ${validation.error}`);
            }
        }
        catch (error) {
            console.warn(`[Admin] Provider validation error for ${name}:`, error);
            validation = {
                valid: false,
...
router.post('/config/import', (req, res) => {
    try {
        ...
        const imported = configService.importConfig(data, strategy);

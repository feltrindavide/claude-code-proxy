...
const urlSchema = z.string()
    .url()
    .refine((url) => url.startsWith('https://') || url.includes('localhost') || url.includes('127.0.0.1'), 'URL must be HTTPS or localhost');
...
export class ConfigService {
     ...
     */
    importConfig(data, strategy) {
        const result = proxyConfigSchema.safeParse(data);
        if (!result.success) {
            throw new Error(`Invalid config: ${result.error.errors.map(e => `${e.path.join('.')}: ${e.message}`).join('; ')}`);
        }
        if (strategy === 'replace') {
            return result.data;
        }

...
export function isTransientError(error) {
    if (error instanceof AbortError)
        return false; // Never retry AbortError (D-67)
 ...
 */
export async function fetchWithRetry(providerName, fn) {
    return pRetry(async (attemptNumber) => {
        ...
        if (response.status >= 400 && response.status < 500) {
            ...
            throw new AbortError(`Provider ${providerName} returned ${response.status}: ${errorText}`);
        }
